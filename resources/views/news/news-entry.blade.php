...
<body>
	...
	<section class="home-section">
        ...
        <form action="{{ url('/news/store') }}" method="post" enctype="multipart/form-data">
            ...
            <textarea class="input" name="isi" id="isi" placeholder="isi">{{ old('isi') }}</textarea>
            @error('isi')
            <p style="font-size: 10px; color: red">{{ $message }}</p>
            @enderror

            <label for="tanggal">Tanggal</label>
            <input class="input" type="date" name="tanggal" id="tanggal" placeholder="tanggal" value="{{ old('tanggal') }}" />
            @error('tanggal')
            <p style="font-size: 10px; color: red">{{ $message }}</p>
            @enderror
